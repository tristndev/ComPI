...
import factories.augmentation.FactorAugmentationFactory;
import factories.base.RandomSampleFactory;

public class FactorAugmentationStrategy extends WorldCreationStrategy {

	private boolean allQueries = true;
	

	// We go through these counts in parallel
	int[] params_logVarCounts = { 3,3,4 }; // # randVarCount / 5
	int[] params_randVarCounts = { 4,5,5 };
	/*
	 * int[] params_randVarCounts = new int[params_logVarCounts.length]; for (int i
	 * = 0; i < params_logVarCounts.length; i++) { params_randVarCounts[i] = (int)
	 * Math.floor(params_logVarCounts[i] * 3); }
	 */
	
	
	int[] params_factorCounts = { 3, 4, 5 };
	/*
	 * int[] params_factorCounts = new int[params_logVarCounts.length]; for (int i =
	 * 0; i < params_logVarCounts.length; i++) { params_factorCounts[i] =
	 * params_randVarCounts[i] + (int) Math.floor(params_logVarCounts[i] * 1.5); }
	 */
	...
	@Override
	public void start() {
		paramHandlerFactorAugm(params_logVarCounts, params_randVarCounts, params_factorCounts, params_argsInFactor,
				params_maxRandVarArgs, params_maxRandVarOcc, params_rerollCount);
	 ...
	 * @param rerollCount
	 */
	public void paramHandlerFactorAugm(int[] logVarCounts, int[] randVarCounts, int[] factorCounts,
			int[] factorArgCounts, int[] maxRandVarArgs, int[] maxRandVarOccurrences, int rerollCount) {

		// Check for equal lengths
		if (!Helpers.checkForEqualLength(new int[][] { randVarCounts, logVarCounts })) {
		...
		for (int r = 0; r < rerollCount; r++) {
			for (int model_i = 0; model_i < randVarCounts.length; model_i++) {
				...
				for (int adv_i = 0; adv_i < factorArgCounts.length; adv_i++) {
					...
					World temp_w = null;
					for (int fac_i = 0; fac_i < factorCounts.length; fac_i++) {
						int n_fac = factorCounts[fac_i];
						...
						if (fac_i == 0) {
							...
							ElementFactory augmentFac = new FactorAugmentationFactory(temp_w);
						...
						ConfigSingle.getInstance().getProgressLogger().logWorldDone(counter, fac_i, temp_w);
						counter++;
					}
				}
			}
